...
static const char *FRAG_SHADER =
    ...
    "uniform float rangeLimit;\n"
    "const float M_PI = 3.14159265358979323846;\n"
    "void main() {\n"
    "  float r0 = length(texc);\n"
    "  if (r0 > 1.0) discard;\n"
    "  float distance = r0 * rangeMax;\n"
    "  if (distance > rangeLimit) discard;\n"
    "  float r = distance / rangeLimit;\n"
...
public:
  OffscreenRadarRenderer(int size, float limit)
    : size_(size), rangeLimit_(limit), currentRangeMax_(limit) {}

  void setRangeMax(float rm) { currentRangeMax_ = rm; }
  ...
  QImage renderFullCircle() {
    ...
    program_->setUniformValue("rangeMax", currentRangeMax_);
...
private:
  int size_;
  float rangeLimit_, currentRangeMax_;
...
void sendRadarStatus(ros::NodeHandle& nh, const std::string& topic, const std::string& status_value) {
  ...
  while (ctrl_pub.getNumSubscribers() == 0 && ros::ok() && tries++ < 50) {
    ROS_INFO_THROTTLE(1.0, "Waiting for %s...", topic.c_str());
    rate.sleep();
  }
  ...
  if (ctrl_pub.getNumSubscribers() > 0) {
    for (int i = 0; i < 5; ++i) {
      ...
      ROS_INFO("Sent [%s] (%d/5)", msg.value.c_str(), i + 1);
      rate.sleep();
    }
  } else {
    ROS_WARN("No subscriber for %s.", topic.c_str());
  }
}
...
int main(int argc, char** argv) {
  ...
  float limit = 2000.0f;
  OffscreenRadarRenderer renderer(512, limit);

  if (!renderer.init()) {
    ROS_ERROR("GL init fail");
    return -1;
  }
  ...
  ros::Subscriber sub = nh.subscribe<marine_sensor_msgs::RadarSector>(
    ...
    [&](const marine_sensor_msgs::RadarSector::ConstPtr& msg) {
      ...
      renderer.setRangeMax(msg->range_max);
      renderer.addSector(*msg);

      if (!first && std::fabs(delta) > WRAP) {
        ROS_INFO_STREAM("Up to " << limit << " m");
